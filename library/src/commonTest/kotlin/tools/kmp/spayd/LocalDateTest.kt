...
class LocalDateTest {
    ...
    fun `test invalid date strings throw IllegalArgumentException`() {
        ...
        assertInvalidDate("", "Date must be exactly 8 digits (YYYYMMDD).")
        assertInvalidDate("2023010", "Date must be exactly 8 digits (YYYYMMDD).")
        assertInvalidDate("202301011", "Date must be exactly 8 digits (YYYYMMDD).")
        assertInvalidDate("2023/01/01", "Date must be exactly 8 digits (YYYYMMDD).")
        assertInvalidDate("abcdefgh", "Date must be exactly 8 digits (YYYYMMDD).")

        // Invalid date value tests
        assertInvalidDate("18991231", "Unreasonable year: 1899.")
        assertInvalidDate("18000101", "Unreasonable year: 1800.")

        // Invalid month tests
        assertInvalidDate("20230000", "Month number must be between 1 and 12.")
        assertInvalidDate("20231300", "Month number must be between 1 and 12.")

        // Invalid day tests
        assertInvalidDate("20230100", "Invalid day of month: 0")
        assertInvalidDate("20230132", "Invalid day of month: 32")
        assertInvalidDate("20230431", "Invalid day of month: 31") // April has 30 days

        // February special cases
        assertInvalidDate("20230229", "Invalid day of month: 29") // Not a leap year
        assertInvalidDate("20230230", "Invalid day of month: 30") // February never has 30 days
        assertInvalidDate("21000229", "Invalid day of month: 29") // 2100 is not a leap year
    }
    ...
    private fun assertInvalidDate(dateString: String, expectedMessage: String) {
        val exception = assertFailsWith<IllegalArgumentException>("failed for input: $dateString") {
            LocalDate.fromString(dateString)
        }
