...
class SpaydTest {
    ...
    fun `bad prefix throws`() {
        ...
        for ((index, invalidString) in invalidStrings.withIndex()) {
            val exception = assertFailsWith<IllegalArgumentException>(
                ...
                block = { SPAYD.parse(invalidString) }
            )
            assertEquals(
    ...
    fun `test missing colon delimiter in key-value pairs`() {
        run {
            ...
            for (case in cases) {
                val exception = assertFailsWith<IllegalArgumentException> { SPAYD.parse(case) }
    ...
    fun `test illegal characters in key-value pair KEYS`() {
        fun performTest(input: String, idx: Int, c: Char, cidx: Int) {
            val ex = assertFailsWith<IllegalArgumentException> { SPAYD.parse(input) }
            val expected =
    ...
    fun `test valid acc`() {
        run {
            val result = SPAYD.parse("$prefix${acc}")
            assertEquals(IbanBic.fromParts(iban, null), result.account)
        }
        run {
            val result = SPAYD.parse("$prefix${accBic}")
            assertEquals(IbanBic.fromParts(iban, bic), result.account)
        }
    }
    ...
    fun `test valid alt-acc`() {
        run {
            val result = SPAYD.parse("$prefix${acc}ALT-ACC:$iban,${iban}+${bic}".also(::println))
            assertEquals(listOf(IbanBic.fromParts(iban, null), IbanBic.fromParts(iban, bic)), result.altAccounts)
        }
    }
    ...
    fun `test real spayd`() {
        ...
        assertEquals("T-Mobile Czech Republic a.s.", result.recipient)
